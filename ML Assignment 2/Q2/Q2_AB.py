...
def main():
    ...
    # load data
    Temp_X_Val = np.array([Temp_X_Value[0] for Temp_X_Value in Portfolio_1_Information])
    Temp_Y_Val = np.array([Temp_X_Value[1] for Temp_X_Value in Portfolio_1_Information])

    # model run
    Regression_Object = Temporary_Local_Weighted_Regression()
    Percepton_model = [Regression_Object.Train_Model(x0, Temp_X_Val, Temp_Y_Val) for x0 in Temp_X_Val]

    # plot
    plt.title("Locally Weighted Linear Regression")
    plt.scatter(Temp_X_Val, Percepton_model, c='b')
    plt.show()
