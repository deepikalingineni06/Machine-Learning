...
def main():
    ...
    # loop to iterate d and k
    for idxP in range(1,11):
        print("Value of K --", idxP)
        for idxd in range(0,7):
            print("Value of D --", idxd)
            
            # prediction
            Mean_Squarred_Rrror_Value = ((np.array([Regression_Method(idxd, idxP, Temp_Value[0]) for Temp_Value in  Portfolio_1_Information]) - np.array([Temp_Value[1] for Temp_Value in  Portfolio_1_Information]))**2).mean()
