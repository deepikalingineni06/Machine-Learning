...
def main():
    ...
    List_of_Features = []
    for Token in range(len(Portfolio_3_Information)):
        Temp_Data = Portfolio_3_Information[Token]
        List_of_Features.append([Temp_Data[1], Temp_Data[2]])
    List_of_Labels = []
    for Token in range(len(Portfolio_3_Information)):
        Temp_Data = Portfolio_3_Information[Token]
        List_of_Labels.append(0 if Temp_Data[3]=='M' else 1)
    List_of_Features = np.array(List_of_Features)
    List_of_Labels = np.array(List_of_Labels)

    # running model
    Trained_Model_Instance = LogidticRegression(Alpha_Value=0.01,Itr_Count=1000)
    Trained_Model_Instance.Train_Model(List_of_Features, List_of_Labels)

    #setting new features for graph
    Axis_1 = [x[0] for x in List_of_Features]
    Axis_2 = [x[1] for x in List_of_Features]
    final_features = []
    for x in np.arange(int(min(Axis_1)-2)+0.0,int(max(Axis_1)+3)+0.0,0.1):
        for y in np.arange(int(min(Axis_2)-2)+0.0,int(max(Axis_2)+3)+0.0,0.1):
            final_features.append([x,y])
    final_features = np.array(final_features)

    # predictions
    pred_original = Trained_Model_Instance.Predict_method(List_of_Features)
    preds_val = Trained_Model_Instance.Predict_method(final_features)

    # plotting
    plt.scatter([ final_features[x][0] for x in range(len(final_features)) if preds_val[x]==1  ], [ final_features[x][1] for x in range(len(final_features)) if preds_val[x]==1  ] )
    plt.scatter([ final_features[x][0] for x in range(len(final_features)) if preds_val[x]==0  ], [ final_features[x][1] for x in range(len(final_features)) if preds_val[x]==0 ], c='red'   )
    plt.scatter([ List_of_Features[x][0] for x in range(len(List_of_Features)) if pred_original[x]==1  ], [ List_of_Features[x][1] for x in range(len(List_of_Features)) if pred_original[x]==1 ], c='red' )
    plt.scatter([ List_of_Features[x][0] for x in range(len(List_of_Features)) if pred_original[x]==0  ], [ List_of_Features[x][1] for x in range(len(List_of_Features)) if pred_original[x]==0 ], c='blue'   )
